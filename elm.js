...
Elm.Thumbs.make = function (_elm) {
   ...
   "Maybe.Maybe Thumbs.Model",
   function (v) {
      return v === null ? Elm.Maybe.make(_elm).Nothing : Elm.Maybe.make(_elm).Just(typeof v === "object" && "ups" in v && "downs" in v && "comments" in v ? {_: {}
                                                                                                                                                            ,ups: typeof v.ups === "number" && isFinite(v.ups) && Math.floor(v.ups) === v.ups ? v.ups : _U.badPort("an integer",
                                                                                                                                                            v.ups)
                                                                                                                                                            ,downs: typeof v.downs === "number" && isFinite(v.downs) && Math.floor(v.downs) === v.downs ? v.downs : _U.badPort("an integer",
                                                                                                                                                            v.downs)
                                                                                                                                                            ,comments: typeof v.comments === "object" && v.comments instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.comments.map(function (v) {
                                                                                                                                                               return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",
                                                                                                                                                               v);
                                                                                                                                                            })) : _U.badPort("an array",
                                                                                                                                                            v.comments)} : _U.badPort("an object with fields `ups`, `downs`, `comments`",
      v));
   });
   ...
   var commentList = function (comments) {
      var commentItem = function (comment) {    return A2($Html.li,_U.list([]),_U.list([$Html.text(comment)]));};
      var commentItems = A2($List.map,commentItem,comments);
      return A2($Html.ul,_U.list([]),commentItems);
   };
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         ...
         case "Down": return _U.update(model,{downs: model.downs + 1});
         default: return _U.update(model,{comments: A2($List._op["::"],_p0._0,model.comments)});}
   });
   var AddComment = function (a) {    return {ctor: "AddComment",_0: a};};
   var Down = {ctor: "Down"};
   var Up = {ctor: "Up"};
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.button,
              ...
              _U.list([$Html.text(A2($Basics._op["++"],$Basics.toString(model.ups)," Thumbs DUp"))]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text($Basics.toString(model))]))
              ,commentList(model.comments)]));
   });
   var NoOp = {ctor: "NoOp"};
   var inbox = $Signal.mailbox(NoOp);
   var actions = A2($Signal.merge,inbox.signal,A2($Signal.map,AddComment,comments));
   var initialModel = function () {
      var emptyModel = {ups: 0,downs: 0,comments: _U.list([])};
      return A4($Debug.log,$Basics.toString($Maybe.Just(getStoredModel)),$Maybe.withDefault,emptyModel,getStoredModel);
   }();
   ...
   function (v) {
      return {ups: v.ups,downs: v.downs,comments: Elm.Native.List.make(_elm).toArray(v.comments).map(function (v) {    return v;})};
   },
   model);
   ...
   var Model = F3(function (a,b,c) {    return {ups: a,downs: b,comments: c};});
