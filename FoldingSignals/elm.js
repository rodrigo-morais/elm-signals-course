...
Elm.Main.make = function (_elm) {
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var reverse = function (original) {    return A2($Signal.map,$String.reverse,original);};
   var characters = A2($Signal.map,$Char.fromCode,$Keyboard.presses);
   var state = A3($Signal.foldp,F2(function ($char,text) {    return A2($String.cons,$char,text);}),"",characters);
   var main = A2($Signal.map,$Graphics$Element.show,reverse(state));
   return _elm.Main.values = {_op: _op,characters: characters,state: state,reverse: reverse,main: main};
};
